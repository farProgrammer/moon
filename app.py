...
def show_homepage():
    ...
    try:
        widget_response = requests.get(
            f"https://api.nasa.gov/insight_weather/?api_key={WAPIKEY}&Last_UTC={today}&feedtype=json&ver=1.0")
        data = widget_response.json()
        # might throw err if not on heroku because api will not recognize key because localhost doesn't provide it, heroku does
        ...
        celsius_on_moon = 'at least -62.2222'
...
def show_logged_in_homepage(user_id):
    ...
    return render_template('logged_in_homepage.html', celsius_on_moon=celsius_on_moon, user=user)
...
def toggle_favorite(photo_id):
    ...
    if favorited_photo in user_faves:
        g.user.favorites = [
            favorite for favorite in user_faves if favorite != favorited_photo]
